...
| UtilityServiceTerritory | The service territory of a utility. When an incentive is associated with a geography of this type, it should be taken to mean that the claimant must be a customer of this utility, rather than that they must live within a specific land area; this is especially relevant when different utilities' territories overlap. The `identifier` field MUST be the `id` of a row in the dataset's `authorities.csv` that represents a utility (i.e.its `level` is `UtilityServiceProvider`).                                                                     |
